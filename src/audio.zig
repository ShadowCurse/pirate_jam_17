...
pub var ear_distance: f32 = 0.01;
pub var max_volume_distance: f32 = 10.0;
pub var forward_volume: f32 = 0.8;

pub const Volume = struct {
    left: f32 = 1.0,
    right: f32 = 1.0,

    pub fn same(v: f32) Volume {
        return .{ .left = v, .right = v };
    }
};
pub const SoundtrackVolumes = std.EnumArray(Assets.SoundtrackType, Volume);

pub const CHANNELS = 2;
pub const FREQUENCY = 44100;

pub const PlayingSoundtrack = struct {
    type: Assets.SoundtrackType = undefined,
...
pub fn callback(
    ...
    for (&playing_soundtracks) |*playing_soundtrack| {
        ...
        if (playing_soundtrack.sound_position) |sound_position| {
            const current_camera = Game.current_camera();
            const forward = current_camera.forward();
            const up = current_camera.up();
            const right = forward.cross(up);
            const left = right.neg();
            const left_ear = current_camera.position.add(left.mul_f32(ear_distance));
            const right_ear = current_camera.position.add(right.mul_f32(ear_distance));
            const left_to_sound = sound_position.sub(left_ear);
            const right_to_sound = sound_position.sub(right_ear);
            const left_distance = left_to_sound.len();
            const right_distance = right_to_sound.len();

            var left_amp = left.dot(left_to_sound.normalize());
            if (0.0 < left_amp)
                left_amp = std.math.clamp(left_amp, forward_volume, 1.0)
            else
                left_amp = (left_amp + 1.0) * forward_volume;

            var right_amp = right.dot(right_to_sound.normalize());
            if (0.0 < right_amp)
                right_amp = std.math.clamp(right_amp, forward_volume, 1.0)
            else
                right_amp = (right_amp + 1.0) * forward_volume;

            const left_volume = (1.0 - @min(left_distance / max_volume_distance, 1.0)) * left_amp;
            const right_volume = (1.0 - @min(right_distance / max_volume_distance, 1.0)) * right_amp;

            playing_soundtrack.left_current_volume = left_volume;
            playing_soundtrack.left_target_volume = left_volume;
            playing_soundtrack.right_current_volume = right_volume;
            playing_soundtrack.right_target_volume = right_volume;
        }
        ...
        const master_volume: @Vector(4, f32) = @splat(global_volume);
        ...
        while (i < rem_i16) : (i += 2) {
            ...
            d_l_f32 *= playing_soundtrack.left_current_volume * global_volume;
            ...
            d_r_f32 *= playing_soundtrack.right_current_volume * global_volume;
...
pub fn init() void {
    ...
    if (sdl.SDL_OpenAudioDeviceStream(
        ...
        log.panic(@src(), "Cannot open audio device: {s}", .{sdl.SDL_GetError()});
    }

    callback_mix_buffer = std.heap.page_allocator.alignedAlloc(
        u8,
        64,
        @sizeOf(i16) * 4096,
    ) catch unreachable;
    callback_track_buffer = std.heap.page_allocator.alignedAlloc(
        u8,
        64,
        @sizeOf(i16) * 4096,
    ) catch unreachable;
...
pub fn play(
    ...
    sound_position: ?*math.Vec3,
) void {
    const soundtrack = Assets.soundtracks.getPtr(soundtrack_type);
    const volume = volumes.getPtr(soundtrack_type);
    for (&playing_soundtracks) |*ps| {
        if (ps.is_finished) {
            ps.* = .{
                ...
                .left_current_volume = volume.left,
                .left_target_volume = volume.left,
                .left_volume_delta_per_sample = 0.0,
                .right_current_volume = volume.right,
                .right_target_volume = volume.right,
...
pub fn imgui_ui() void {
    ...
    if (cimgui.igCollapsingHeader_BoolPtr(
        ...
        cimgui.format("Global volume", &global_volume);
        cimgui.format("Ear distance", &ear_distance);
        cimgui.format("Max volume distance", &max_volume_distance);

        cimgui.format("Per type volume", &volumes);
